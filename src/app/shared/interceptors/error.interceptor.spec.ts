...
describe('ErrorInterceptor', () => {
    ...
    it('debe procesar el mensaje de error y notificar al servicio', (done) => {
        ...
        interceptor.intercept(new HttpRequest('GET', '/'), handler).subscribe({
            next: () => {},
            error: () => {
                expect(actualizar.mensajeError).toHaveBeenCalledWith(
                    'Error en el envío de los datos',
                );
                done();
            },
        });
    ...
    it('procesarErrorMessage cubre todas las ramas', () => {
        expect(ErrorInterceptor.procesarErrorMessage({ status: 401 })).toBe(
            'El usuario no inició sesión',
        );
        expect(ErrorInterceptor.procesarErrorMessage({ status: 403 })).toBe(
            'El usuario no está autorizado',
        );
        expect(
            ErrorInterceptor.procesarErrorMessage({
                status: 409,
                error: { mensajes: [{ descripcion: 'x' }] },
            }),
        ).toBe('x');
        expect(
            Array.isArray(
        ...
        expect(ErrorInterceptor.procesarErrorMessage({ status: 500 })).toBe(
            'Error interno del servidor',
        );
        expect(ErrorInterceptor.procesarErrorMessage({ status: 0 })).toBe(
