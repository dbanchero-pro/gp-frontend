...
describe('GcRestService', () => {
  ...
  beforeEach(() => {
    ...
    restApi = jasmine.createSpyObj('RestApiService', ['get', 'post', 'put', 'patch', 'delete']);
  ...
  it('get concatena la URL base', () => {
    service.get('/x');
    expect(restApi.get).toHaveBeenCalledWith('/api/x', jasmine.any(HttpParams));
  });

  it('post concatena la URL base', () => {
    service.post('/x', { a: 1 });
    expect(restApi.post).toHaveBeenCalledWith('/api/x', { a: 1 }, jasmine.any(HttpParams));
  });
