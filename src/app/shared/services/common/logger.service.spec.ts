...
describe('LoggerService', () => {
    ...
    it('determinarLoggingLevel convierte correctamente las cadenas', () => {
        ...
        expect(service.determinarLoggingLevel('error')).toBe(
            LoggingLevel.Error,
        );
        expect(service.determinarLoggingLevel('warning')).toBe(
            LoggingLevel.Warning,
        );
        expect(service.determinarLoggingLevel('info')).toBe(LoggingLevel.Info);
        expect(service.determinarLoggingLevel('debug')).toBe(
            LoggingLevel.Debug,
        );
        expect(service.determinarLoggingLevel('verbose')).toBe(
            LoggingLevel.Verbose,
        );
        expect(service.determinarLoggingLevel(undefined as any)).toBe(
            LoggingLevel.None,
        );
    });
    ...
    it('debería llamar a los métodos de consola según el nivel de log', () => {
        ...
        expect(service.determinarLoggingLevel('whatever')).toBe(
            LoggingLevel.Verbose,
        );
    });
