...
class KeycloakServiceMock {
  ...
  getToken = jasmine.createSpy('getToken').and.returnValue(Promise.resolve('tok'));
  isLoggedIn = jasmine.createSpy('isLoggedIn').and.returnValue(true);
  updateToken = jasmine.createSpy('updateToken').and.returnValue(Promise.resolve(true));
  loadUserProfile = jasmine.createSpy('loadUserProfile').and.returnValue(Promise.resolve({} as any));
}
