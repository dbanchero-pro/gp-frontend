...
describe('SeguridadService', () => {
    ...
    it('debería cargar permisos del servicio cuando no están guardados', async () => {
        ...
        spyOn(util, 'usuarioInfo').and.returnValue(
            of({ permisos: ['A', 'B'] } as any),
        );
        sessionStorage.removeItem('permisos');
    ...
    it('cargarContexto almacena la información del usuario', async () => {
        ...
        expect(sessionStorage.getItem('proveedores')).toBe(
            JSON.stringify(info.proveedores),
        );
    });
    ...
    it('usuarioLogueadoPuedeCambiarPerfil es verdadero si tiene ambos', () => {
        spyOn(servicio, 'usuarioLogueadoEsUsuarioOrganismo').and.returnValue(
            true,
        );
        spyOn(servicio, 'usuarioLogueadoEsUsuarioProveedor').and.returnValue(
            true,
        );
