...
describe('CompraSiceService', () => {
  ...
  it('mapearColumnaOrdenamiento mapea claves conocidas', () => {
    expect((service as any).mapearColumnaOrdenamiento('idUnidadCompra')).toBe('unidadCompra.idUnidadCompra');
  });

  it('mapearColumnaOrdenamiento mapea columnas conocidas y valor por defecto', () => {
    const service = new CompraService({} as any);
    expect((service as any).mapearColumnaOrdenamiento('idInciso')).toBe('unidadCompra.id.unidadEjecutora.id.inciso.id');
    expect((service as any).mapearColumnaOrdenamiento('unknown')).toBe('unidadCompra.id.unidadEjecutora.id.inciso.id');
  });
  ...
  it('obtenerItemsCompra debería llamar al endpoint correcto', () => {
    ...
    expect(rest.lastUrl).toBe('/api/gestion-contratos/v1/compra-sice/item/all');
  ...
  it('_buildHttpParamsDesdeFiltro ignora valores vacios', () => {
    const params = (service as any)._buildHttpParamsDesdeFiltro({ idInciso: 1, numCompra: '', idUsuario: null });
  ...
  it('_buildHttpParams arma sort correcto', () => {
    const params = (service as any)._buildHttpParams({ filtro: null, page: 1, size: 5, sort: 'idInciso', order: 'desc' });
