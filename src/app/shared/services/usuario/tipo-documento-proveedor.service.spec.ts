...
describe('TipoDocumentoProveedorService', () => {
  ...
  it('obtiene tipos de documento con filtros', () => {
    gcRestSpy.get.and.returnValue(of({} as PageModel<ITipoDocumentoProveedorDTO>));
    service.obtenerTiposDocumentoProveedor(1, 50, 'id,asc', Pais.URUGUAY, TipoDocumentoUsuario.CEDULA_IDENTIDAD).subscribe();
  ...
  it('obtiene todos los documentos', () => {
    const mock: PageModel<ITipoDocumentoProveedorDTO> = {
      content: [{ tipoDocumento: TipoDocumentoUsuario.CEDULA_IDENTIDAD }] as any,
    } as any;
    gcRestSpy.get.and.returnValue(of(mock));
    service.obtenerTodos().subscribe(res => {
      expect(res[0].tipoDocumento).toBe(TipoDocumentoUsuario.CEDULA_IDENTIDAD);
    });
  });
});
