...
export class PaisService {
  ...
  obtenerPaises(page: number = 0, size: number = 20, sort: string = 'id,asc'): Observable<PageModel<IPaisDTO>> {
    const params = new HttpParams()
      .set('page', page)
      .set('size', size)
      .set('sort', sort);

    return this.gcRestService.get<PageModel<IPaisDTO>>(`${this.baseUrl}/all`, params);
  }
