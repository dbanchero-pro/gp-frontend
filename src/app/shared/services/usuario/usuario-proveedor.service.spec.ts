...
describe('UsuarioProveedorService', () => {
    ...
    it('debería guardar un usuario proveedor', () => {
        ...
        expect(gcRestSpy.post).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-proveedor',
            dto,
        );
    });
    ...
    it('debería eliminar un usuario proveedor', () => {
        ...
        expect(gcRestSpy.delete).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-proveedor/maria/UY/CI/12345678',
        );
    });
    ...
    it('exportarExcelUsuariosProveedor genera y descarga archivo', () => {
        const spyCreate = spyOn(document, 'createElement').and.callFake(
            (): any => ({ click: jasmine.createSpy('click') }),
        );
        const spyUrl = spyOn(window.URL, 'createObjectURL').and.returnValue(
            'blob:url',
        );
        gcRestSpy.post.and.returnValue(
            of({
                contenido: btoa('data'),
                mimeType: 'text/plain',
                nombre: 'file.txt',
        ...
        expect(gcRestSpy.post).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-proveedor/excel',
            { nroDocumento: '1' },
        );
    ...
    it('exportarExcelUsuariosProveedor con filtros vacíos no envía parámetros', () => {
        gcRestSpy.post.and.returnValue(
            of({ contenido: btoa('x'), mimeType: 'x', nombre: 'f' } as any),
        );
        service.exportarExcelUsuariosProveedor({});
    ...
    it('exportarExcelUsuariosProveedorUsuarioOrganismo llama endpoint adecuado', () => {
        ...
        service.exportarExcelUsuariosProveedorUsuarioOrganismo({
            idPais: Pais.URUGUAY,
        });
        expect(gcRestSpy.post).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-proveedor/excel-organismo',
            { idPais: Pais.URUGUAY },
        );
    });
    ...
    it('actualizarUsuarioProveedor llama al endpoint correcto', () => {
        ...
        expect(gcRestSpy.put).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-proveedor/u1/UY/CI/2',
            dto,
        );
    });
});
