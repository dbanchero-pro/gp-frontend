...
describe('UsuarioOrganismoService', () => {
    ...
    it('debería obtener un usuario de organismo por ID compuesto', () => {
        ...
        expect(gcRestSpy.get).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-organismo/maria/1/2/3',
        );
    });
    ...
    it('obtenerUsuariosOrganismoNoExiste envia filtros', () => {
        gcRestSpy.get.and.returnValue(of([]));
        service
            .obtenerUsuariosOrganismoNoExiste('CONFORMIDAD' as any, {
                idInciso: 1,
                idUnidadEjecutora: 2,
            })
            .subscribe();
        expect(gcRestSpy.get).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-organismo/usuarios-no-existentes-gc-conformidad',
            jasmine.anything(),
        );
    });
    ...
    it('obtenerInformacionUsuarioSice incluye params', () => {
        ...
        service
            .obtenerInformacionUsuarioSice('u1', 'CONFORMIDAD' as any)
            .subscribe();
        expect(gcRestSpy.post).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/usuarios-organismo/obtener-informacion-usuario-sice',
            null,
            jasmine.anything(),
        );
    });
