...
describe('TipoDocumentoUsuarioService', () => {
  ...
  it('debería obtener tipos de documento con filtros', () => {
    gcRestSpy.get.and.returnValue(of({} as PageModel<ITipoDocumentoUsuarioDTO>));
    service.obtenerTiposDocumentoUsuario(1, 50, 'nombre,asc', Pais.URUGUAY, TipoDocumentoUsuario.CEDULA_IDENTIDAD).subscribe();
  ...
  it('debería obtener todos los tipos de documento', () => {
    const mockPage: PageModel<ITipoDocumentoUsuarioDTO> = {
      content: [{ idTipoDocumento: TipoDocumentoUsuario.CEDULA_IDENTIDAD, descripcion: 'Cédula' }] as ITipoDocumentoUsuarioDTO[],
      totalElements: 1,
      totalPages: 1,
      number: 0,
      size: 1000,
    ...
    service.obtenerTodos().subscribe(result => {
      ...
      expect(result[0].idTipoDocumento).toBe(TipoDocumentoUsuario.CEDULA_IDENTIDAD);
    });
