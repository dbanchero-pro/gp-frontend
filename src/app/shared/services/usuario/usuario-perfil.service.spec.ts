...
class MockGcRestService {
  lastUrl = '';
  lastParams: any;
  get(url: string, params: any) { this.lastUrl = url; this.lastParams = params; return of(null); }
  post(url: string, data: any, params: any) { this.lastUrl = url; this.lastParams = params; return of(true); }
  delete(url: string) { this.lastUrl = url; return of(true); }
}
...
describe('UsuarioOrganismoPerfilService', () => {
  ...
  beforeEach(() => {
    ...
    service = new UsuarioOrganismoPerfilService(rest as any, archivoService as any);
  });

  it('obtenerTodos debería enviar parámetros', () => {
    service.obtenerTodos({ nroDocumento: '1', idInciso: 2 }, 1, 5, 'id,asc').subscribe();
    expect(rest.lastUrl).toBe('/api/gestion-contratos/v1/usuarios-organismo-perfil/all');
  ...
  it('guardar debería enviar el DTO por POST', () => {
    ...
    expect(rest.lastUrl).toBe('/api/gestion-contratos/v1/usuarios-organismo-perfil');
  });
  ...
  it('otros métodos del servicio llaman a las URLs correctas', () => {
    ...
    expect(rest.lastUrl).toContain('/api/gestion-contratos/v1/usuarios-organismo-perfil/9');
  });
  ...
  it('exportarUsuariosPerfil debería crear un blob', () => {
    ...
    spyOn(rest, 'post').and.callFake((url: string, data: any, params: any) => {
      rest.lastUrl = url; rest.lastParams = params; return of({ contenido: btoa('x'), mimeType: 'text/plain', nombre: 'a.txt' } as any); });

    service.exportarUsuariosPerfil({ idInciso: 1 });
    expect(rest.lastUrl).toContain('/api/gestion-contratos/v1/usuarios-organismo-perfil/excel');
  });

  it('buscarArticulos incluye filtros', () => {
    spyOn(rest, 'get').and.callThrough();
    service.buscarArticulos('CONFORMIDAD', { numCompra: '1', descripcionArticulo: 'x' }).subscribe();
  ...
  it('buscarArticulos cubre otros campos', () => {
    spyOn(rest, 'get').and.callThrough();
    service.buscarArticulos('CONFORMIDAD', { anioCompra: '2024', nroItem: '2', filtrarPorArticulo: true }).subscribe();
    expect(rest.get).toHaveBeenCalled();
    expect(rest.lastParams.get('anioCompra')).toBe('2024');
