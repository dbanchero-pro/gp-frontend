...
describe('ItemsCompraService', () => {
    ...
    it('modificar llama a put con la URL correcta', () => {
        ...
        expect(rest.put).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/items-compra/1/2',
            { x: 1 } as any,
        );
    });
    ...
    it('eliminar llama a delete con la URL correcta', () => {
        ...
        expect(rest.delete).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/items-compra/eliminar/1/2',
        );
    });
    ...
    it('copiar llama a put con cuerpo nulo', () => {
        service.copiar(1, 2);
        expect(rest.put).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/items-compra/copiarItem/1/2',
            null,
        );
    });
    ...
    it('buscarPorArticulo usa el filtro según el tipo', () => {
        ...
        expect(rest.get).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/items-compra/filtrar-articulos/1/a',
        );
        service.buscarPorArticulo(1, 'a', TipoBusqueda.NROITEM);
        expect(rest.get).toHaveBeenCalledWith(
            '/api/gestion-contratos/v1/items-compra/filtrar-nroitem/1/a',
        );
    });
});
