...
describe('FiltroOrganismo', () => {
    ...
    beforeEach(async () => {
        mockOrganismoService = {
            obtenerIncisos: jasmine
                .createSpy()
                .and.returnValue(of([{ id: 1, nombre: 'Inciso 1' }])),
            obtenerUE: jasmine
                .createSpy()
                .and.returnValue(
                    of([{ idUnidadEjecutora: 10, nombre: 'UE 1' }]),
                ),
            obtenerUC: jasmine
                .createSpy()
                .and.returnValue(of([{ idUnidadCompra: 100, nombre: 'UC 1' }])),
        };

        await TestBed.configureTestingModule({
            imports: [ReactiveFormsModule, FiltroOrganismoComponent],
            declarations: [],
            providers: [
    ...
    it('debería cargar unidades ejecutoras al seleccionar un inciso', () => {
        ...
        expect(mockOrganismoService.obtenerUE).toHaveBeenCalledWith(
            1,
            false,
            undefined,
        );
        expect((component as any).form.get('idInciso')?.value).toBe(1);
        expect((component as any).form.get('idUnidadEjecutora')?.value).toBe(
            10,
        );
    });
    ...
    it('debería cargar unidades de compra al seleccionar una unidad ejecutora', () => {
        ...
        expect(mockOrganismoService.obtenerUC).toHaveBeenCalledWith(
            1,
            10,
            false,
            undefined,
    ...
    it('validate marca controles y retorna false si están vacíos', () => {
        ...
        expect(
            (component as any).form.get('idUnidadEjecutora')?.touched,
        ).toBeTrue();
        expect(
            (component as any).form.get('idUnidadCompra')?.touched,
        ).toBeTrue();
    });
    ...
    it('debería manejar error al cargar UE o UC', () => {
        mockOrganismoService.obtenerUE.and.returnValue(
            throwError(() => new Error('Error UE')),
        );
    ...
    it('aplicarValidadores establece los validadores requeridos', () => {
        ...
        expect(
            (component as any).form.get('idUnidadEjecutora')?.validator,
        ).toBeTruthy();
        expect(
            (component as any).form.get('idUnidadCompra')?.validator,
        ).toBeTruthy();
    });
