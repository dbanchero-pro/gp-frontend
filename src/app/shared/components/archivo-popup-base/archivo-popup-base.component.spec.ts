...
describe('ArchivoPopupBaseComponent', () => {
  ...
  beforeEach(() => {
    ...
    AppConfig.settings = { extensionesPermitidas: 'pdf', archivosTamanoMaxBytes: 1000 } as any;
  });
  ...
  it('actualizarTextoErrorSize arma el mensaje correcto', () => {
    ...
    expect(comp.errorMaxSize).toBe('El tamaño del archivo es de 512 Bytes y el máximo permitido es de 1000 Bytes.');
  });
  ...
  it('onArchivoSeleccionadoInterno procesa archivo válido', () => {
    ...
    comp.onArchivoSeleccionadoInterno({ target: { files: [file] } }, accion);
  ...
  it('onArchivoSeleccionadoInterno marca error si el archivo es grande', () => {
    const file = new File(['a'.repeat(2048)], 'grande.txt', { type: 'text/plain' });
    const mockReader = {
    ...
    spyOn(window as any, 'FileReader').and.returnValue(mockReader);
    comp.onArchivoSeleccionadoInterno({ target: { files: [file] } }, () => {});
