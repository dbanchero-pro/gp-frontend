...
export class TabsComponent implements AfterContentInit {
  ...
  get enabledTabs(): number[] {
    return this.tabs
      ? this.tabs.toArray().map((t, i) => (t.deshabilitado ? -1 : i)).filter(i => i >= 0)
      : [];
  }
  ...
  private ensureValidSelection(): void {
    ...
    if (this.selectedIndex < 0
      ...
      const firstEnabled = this.tabs.toArray().findIndex(t => !t.deshabilitado);
  ...
  handleKeydown(e: KeyboardEvent) {
    ...
    switch (e.key) {
      case 'ArrowRight':
        ...
        break;
      case 'ArrowLeft':
        nextIndex = enabled[(currentPos - 1 + enabled.length) % enabled.length];
        break;
      case 'Home':
        nextIndex = enabled[0];
        break;
