...
@Component({
    selector: 'app-consulta-usuarios-conformidad-items',
    templateUrl: './consulta-usuarios-conformidad-items.component.html',
    standalone: false,
})
export class ConsultaUsuariosConformidadItemsComponent extends PaginaBusquedaComponent<IConsultaUsuarioOrganismoPerfilFiltroDTO>
    implements OnInit {
    ...
    volver() {
        this.router.navigate(['/administracion/gestion-usuarios/consulta-usuario-conformidad', this.idUsuario],{queryParams: { volver: '1' },});
    }
    ...
    obtenerAccionesItem(item: ItemCompraDto): AccionBoton[] {
        const acciones: AccionBoton[] = [
            {
                nombre: 'Agregar',
                ariaLabel: "Agregar permiso por item " + item.nroItem + " usuario id " + this.usuario.id,
    ...
    agregarPermisoPorItem(item: ItemCompraDto): void {
        this.actualizarServ.confirmar('¿Está seguro que desea agregar el permiso?',
            () => {
                const idCompra = this.compra?.idCompra;
                const idUsuario = this.usuario?.id;
                const idItem = item.idItem;
                if (idCompra && idUsuario && idItem) {
                    this.usuarioOrganismoPerfilService.agregarConformidadPorItem(idCompra, idItem, idUsuario)
                        .subscribe(() => {
