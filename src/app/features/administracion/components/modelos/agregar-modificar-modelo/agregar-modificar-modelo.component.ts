...
export class AgregarModificarModeloComponent
    ...
    implements OnInit, CanComponentDeactivate
{
        ...
        setTimeout(() => {
            ...
            if (state && state['seccionSeleccionada']) {
                ...
                if (historyState?.seccionSeleccionada) {
                    this.agregarSeccionDesdeSeleccion(
                        historyState.seccionSeleccionada,
                    );
    ...
    private agregarSeccionDesdeSeleccion(
        seccion: ModeloSeccionDTO | SeccionDTO,
    ): void {
        const seccionNormalizada = this.normalizarSeccionSeleccionada(seccion);
        const seccionId = seccionNormalizada.seccion?.id;
        const yaExiste = this.seccionesAgregadas.some(
            (s) => s.seccion?.id === seccionId,
        );

        if (yaExiste) {
            this.actualizarService.mensajeError(
                'La sección ya está agregada al modelo',
            );
            return;
        }
    ...
    private normalizarSeccionSeleccionada(
        seccion: ModeloSeccionDTO | SeccionDTO,
    ): ModeloSeccionDTO {
    ...
    eliminarSeccion(seccion: ModeloSeccionDTO): void {
        const index = this.seccionesAgregadas.findIndex(
            (s) => s.seccion?.id === seccion.seccion?.id,
        );
        if (index > -1) {
    ...
    moverSeccionArriba(seccion: ModeloSeccionDTO): void {
        ...
        if (index > 0) {
            [
                this.seccionesAgregadas[index - 1],
                this.seccionesAgregadas[index],
            ] = [
                this.seccionesAgregadas[index],
                this.seccionesAgregadas[index - 1],
            ];
    ...
    obtenerAccionesSeccion(seccion: ModeloSeccionDTO): AccionBoton[] {
        ...
        if (index > 0) {
            acciones.push({
                ...
                ariaLabel:
                    'Subir sección ' + seccion.seccion?.denominacion || '',
        ...
        if (index < this.seccionesAgregadas.length - 1) {
            acciones.push({
                ...
                ariaLabel:
                    'Bajar sección ' + seccion.seccion?.denominacion || '',
        ...
        acciones.push({
            ...
            icono: 'fa fa-trash',
            ariaLabel:
                'Eliminar sección ' + seccion.seccion?.denominacion || '',
    ...
    agregarTipoCompra(): void {
        ...
        if (!tipoCompraId) {
            this.actualizarService.mensajeError(
                'Debe seleccionar un tipo de compra',
            );
            return;
        }
    ...
    guardar(): void {
        ...
        if (this.form.errors?.['fechasInvalidas']) {
            this.actualizarService.mensajeError(
                'La fecha vigencia desde debe ser anterior a la fecha vigencia hasta',
            );
            return;
        }
        ...
        const unidadEjecutora = valores.unidadEjecutoraId
            ? this.unidadesEjecutorasMock.find(
        ...
        const operacion = this.modoIngreso
            ...
            : this.modeloService.actualizarModelo(
                  this.idModelo,
                  modelo as ModeloDTO,
              );
        ...
        operacion.subscribe({
            ...
            error: (err) => {
                if (err?.error?.mensaje?.includes('denominación')) {
                    this.actualizarService.mensajeError(
                        'Ya existe un modelo con esta denominación',
                    );
                } else {
                    this.actualizarService.mensajeError(
    ...
    volver(): void {
        this.router.navigate(['/administracion/modelos'], {
