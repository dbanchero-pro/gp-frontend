...
export class AgregarModificarReglaPopupComponent
    ...
    implements OnInit
{
    ...
    override ngOnInit(): void {
        ...
        this.operadores = this.operadorHelper.obtenerOperadoresPorTipoDato(
            this.tipoDatoCampo,
        );
    ...
    private inicializarFormulario(): void {
        this.form = this.fb.nonNullable.group({
            codigo: this.fb.nonNullable.control<string>(
                '',
                Validators.required,
            ),
            ...
            tipoRegla: this.fb.nonNullable.control<string>(
                '',
                Validators.required,
            ),
            operador: this.fb.nonNullable.control<string>(
                '',
                Validators.required,
            ),
            ...
            mensajeError: this.fb.nonNullable.control<string>(
                '',
                Validators.required,
            ),
        });
    ...
    private cargarCamposDisponibles(): void {
        this.campoService
            .obtenerCamposPorTipo(this.tipoDatoCampo, this.idCampoActual)
            .subscribe({
                next: (campos) => {
    ...
    private cargarDatosRegla(): void {
        ...
        if (
            this.reglaExistente.operador === OperadorRegla.RANGO &&
            Array.isArray(this.reglaExistente.valor)
        ) {
            valorRangoInicio = this.reglaExistente.valor[0]?.toString() || '';
            valorRangoFin = this.reglaExistente.valor[1]?.toString() || '';
        } else if (
            this.reglaExistente.operador === OperadorRegla.LISTA_VALORES &&
            Array.isArray(this.reglaExistente.valor)
        ) {
        ...
        this.form.patchValue({
            ...
            idCampoComparar:
                this.reglaExistente.idCampoComparar?.toString() || '',
    ...
    esOperadorSimple(): boolean {
        ...
        return (
            operador !== OperadorRegla.RANGO &&
            operador !== OperadorRegla.LISTA_VALORES
        );
    }
    ...
    guardar(): void {
        ...
        const codigoExiste = this.reglasExistentes.some(
            (r) => r.codigo === codigo,
        );
        if (codigoExiste) {
        ...
        const nombreExiste = this.reglasExistentes.some(
            (r) => r.nombre === nombre,
        );
        if (nombreExiste) {
            ...
            if (operador === OperadorRegla.RANGO) {
                ...
                if (this.tipoDatoCampo === TipoDatoCampo.NUMERO) {
                    ...
                    if (isNaN(inicioNum) || isNaN(finNum)) {
                        this.mostrarError(
                            'Los valores del rango deben ser numéricos',
                        );
                        return;
                    }
                    if (inicioNum >= finNum) {
                ...
                const lista = (this.form.value.valorLista || '')
                    .split(';')
                    .map((v) => v.trim())
                    .filter((v) => v);
                if (lista.length === 0) {
                    this.mostrarError(
                        'Debe ingresar al menos un valor en la lista',
                    );
                    return;
                }
                ...
                if (listaUnica.length !== lista.length) {
                    this.mostrarError(
                        'Los valores de la lista no se pueden repetir',
                    );
                    return;
                }
                valor = lista;
