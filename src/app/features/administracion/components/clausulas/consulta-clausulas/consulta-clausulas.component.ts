...
export class ConsultaClausulasComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        ...
        this.idCapituloOrigen =
            this.route.snapshot.queryParamMap.get('idCapitulo');
    ...
    obtenerAccionesClausula(clausula: ClausulaDTO): AccionBoton[] {
        ...
        acciones.push({
            ...
            ariaLabel:
                'Ver diferencias con versión anterior ' + clausula.denominacion,
        ...
        acciones.push({
            ...
            icono: 'fa fa-list',
            ariaLabel:
                'Ver modelos que usan la cláusula ' + clausula.denominacion,
        ...
        if (this.origenNavegacion === 'capitulo') {
            if (this.idCapituloOrigen && this.idCapituloOrigen !== 'nuevo') {
                this.router.navigate([
                    '/administracion/capitulos/modificar',
                    this.idCapituloOrigen,
                ]);
            } else {
            ...
            this.clausulaService.eliminarClausula(clausulaId).subscribe({
                ...
                error: () => {
                    this.actualizarService.mensajeError(
                        'Ocurrió un error al eliminar la cláusula.',
                    );
                },
            });
        });
    ...
    verHistorial(clausula: ClausulaDTO): void {
        ...
        this.router.navigate([
            '/administracion/clausulas/historial',
            clausula.id,
        ]);
    }
    ...
    verDiferencias(clausula: ClausulaDTO): void {
        ...
        this.router.navigate([
            '/administracion/clausulas/diferencias',
            clausula.id,
        ]);
    }
    ...
    verModelos(clausula: ClausulaDTO): void {
        ...
        this.router.navigate([
            '/administracion/clausulas/modelos',
            clausula.id,
        ]);
    }
    ...
    obtenerEstadoVigencia(clausula: ClausulaDTO): string {
        if (clausula.estado === 'BORRADOR') {
            const hoy = new Date();
            const desde = clausula.fechaVigenciaDesde
                ? new Date(clausula.fechaVigenciaDesde)
                : null;
            const hasta = clausula.fechaVigenciaHasta
                ? new Date(clausula.fechaVigenciaHasta)
                : null;
