...
export class AgregarModificarClausulaComponent
    ...
    implements OnInit, CanComponentDeactivate
{
    ...
    private inicializarFormularios(): void {
        ...
        this.form = this.fb.nonNullable.group({
            ...
            fechaVigenciaDesde: this.fb.nonNullable.control<string>(
                this.modoIngreso ? hoy : '',
                Validators.required,
            ),
            ...
            esObligatoria: this.fb.nonNullable.control<string>(
                SiNoValor.SI,
                Validators.required,
            ),
            aperturaElectronica: this.fb.nonNullable.control<string>(
                SiNoAmbasValor.NO,
                Validators.required,
            ),
        });
    ...
    private configurarCambiosFiltros(): void {
        ...
        this.formTipoCompra
            ...
            ?.valueChanges.subscribe((tipoCompraId) => {
                this.subtiposCompra = tipoCompraId
                    ? this.subtiposCompraBase.filter(
                          (st) => st.idTipoCompra === tipoCompraId,
                      )
                    : [];
                this.formTipoCompra.patchValue({ subtipoCompraId: null });
            });

        this.formObjetoCompra
            .get('familiaId')
            ?.valueChanges.subscribe((familiaId) => {
                this.subfamilias = familiaId
                    ? this.subfamiliasBase.filter(
                          (sf) => sf.familiaId === String(familiaId),
                      )
                    : [];
        ...
        this.formObjetoCompra
            .get('subfamiliaId')
            ?.valueChanges.subscribe((subfamiliaId) => {
                this.clases = subfamiliaId
                    ? this.clasesBase.filter(
                          (c) => c.subfamiliaId === subfamiliaId,
                      )
                    : [];
                this.formObjetoCompra.patchValue({
                    claseId: null,
        ...
        this.formObjetoCompra
            .get('claseId')
            ?.valueChanges.subscribe((claseId) => {
                this.subclases = claseId
                    ? this.subclasesBase.filter(
                          (sc) => sc.claseId === String(claseId),
                      )
                    : [];
                this.formObjetoCompra.patchValue({
                    subclaseId: null,
        ...
        this.formObjetoCompra
            .get('subclaseId')
            ?.valueChanges.subscribe((subclaseId) => {
                this.articulos = subclaseId
                    ? this.articulosBase.filter(
                          (art) => art.subclase?.id === subclaseId,
                      )
                    : [];
        ...
        this.clausulaService.obtenerClausulaPorId(this.idClausula).subscribe({
            next: (clausula: ClausulaDTO | undefined) => {
                ...
                this.form.patchValue({
                    ...
                    fechaVigenciaDesde: convertirFecha(
                        clausula.fechaVigenciaDesde,
                    ),
                    fechaVigenciaHasta: convertirFecha(
                        clausula.fechaVigenciaHasta,
                    ),
                    incisoId: null,
                    unidadEjecutoraId: null,
                    esObligatoria: clausula.obligatoria
                        ? SiNoValor.SI
                        : SiNoValor.NO,
                    aperturaElectronica: clausula.aperturaElectronica
                        ? SiNoValor.SI
                        : SiNoValor.NO,
                });
    ...
    agregarTipoCompra(): void {
        ...
        const tipoCompra = this.tiposCompra.find(
            (tc) => tc.id === tipoCompraId,
        );
        if (!tipoCompra) {
            return;
        }
        ...
        const subtipoCompra = subtipoCompraId
            ? this.subtiposCompra.find(
                  (st) => st.idSubtipoCompra === subtipoCompraId,
              )
            : undefined;

        const yaExiste = this.tiposCompraAgregados.some(
            (tc) =>
        ...
        if (yaExiste) {
            this.actualizarService.mensajeError(
                'Este tipo y subtipo de compra ya fue agregado',
            );
            return;
        }
    ...
    eliminarTipoCompra(tipoCompra: TipoCompraClausulaModeloDTO): void {
        const index = this.tiposCompraAgregados.findIndex(
            (tc) =>
                tc.tipoCompra?.id === tipoCompra.tipoCompra?.id &&
                tc.subtipoCompra?.idSubtipoCompra ===
                    tipoCompra.subtipoCompra?.idSubtipoCompra,
        );
        if (index > -1) {
    ...
    agregarObjetoCompra(): void {
        ...
        const subfamilia = valores.subfamiliaId
            ? this.subfamilias.find((sf) => sf.id === valores.subfamiliaId)
            : undefined;
        const clase = valores.claseId
            ? this.clases.find((c) => c.id === valores.claseId)
            : undefined;
        const subclase = valores.subclaseId
            ? this.subclases.find((sc) => sc.id === valores.subclaseId)
            : undefined;
        const articulo = valores.articuloId
            ? this.articulos.find((a) => a.id === valores.articuloId)
            : undefined;
        ...
        const yaExiste = this.objetosCompraAgregados.some(
            (oc) =>
        ...
        if (yaExiste) {
            this.actualizarService.mensajeError(
                'Este objeto de compra ya fue agregado',
            );
            return;
        }
    ...
    eliminarObjetoCompra(objeto: ObjetoCompraDTO): void {
        const index = this.objetosCompraAgregados.findIndex(
            (oc) =>
        ...
        const procesarNodo = (nodo: Node): boolean => {
            ...
            if (nodo.nodeType === Node.TEXT_NODE) {
                ...
                if (textoNodo.length <= espacioRestante) {
                    ...
                    resultadoHTML +=
                        textoNodo.substring(0, espacioRestante) + '...';
    ...
    guardar(): void {
        ...
        const inciso =
            this.incisos.find((i) => i.id === valores.incisoId) || null;
        const unidadEjecutora = valores.unidadEjecutoraId
            ? this.unidadesEjecutorasBase.find(
                  (ue) => ue.id === valores.unidadEjecutoraId,
              ) || null
            : null;
        ...
        const operacion = this.modoIngreso
            ...
            : this.clausulaService.actualizarClausula(
                  this.idClausula,
                  clausula as ClausulaDTO,
              );
            ...
            next: () => {
                this.actualizarService.mensajeCorrecto(
                    this.modoIngreso
                        ...
                        : 'Cláusula actualizada exitosamente',
                );
                this.form.markAsPristine();
                this.volver();
            },
    ...
    aprobar(): void {
        this.actualizarService.confirmar(
            ...
            () => {
                this.clausulaService
                    .aprobarClausula(this.idClausula)
                    .subscribe({
                        next: () => {
                            this.actualizarService.mensajeCorrecto(
                                'Cláusula aprobada exitosamente',
                            );
                            this.form.markAsPristine();
                            this.volver();
                        },
    ...
    canDeactivate(): boolean {
        return (
            !this.form.dirty &&
            !this.formTipoCompra.dirty &&
            !this.formObjetoCompra.dirty
        );
    }
}
