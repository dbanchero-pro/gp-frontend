...
export class DiferenciasClausulasComponent implements OnInit {
    ...
    cargarDiferencias(): void {
        ...
        forkJoin({
            ...
            anterior: this.clausulaService.obtenerVersionAnterior(
                this.clausulaId,
            ),
        }).subscribe({
            next: ({ actual, anterior }) => {
                ...
                if (!this.versionActual) {
                    ...
                    this.mensajeError =
                        'No se encontró la cláusula seleccionada.';
                } else if (!this.versionAnterior) {
                    this.errorCarga = true;
                    this.mensajeError =
                        'No existe una versión anterior para comparar.';
                } else {
            ...
            error: () => {
                this.errorCarga = true;
                this.mensajeError =
                    'Ocurrió un error al cargar las versiones de la cláusula.';
                this.cargando = false;
            },
        });
    }
    ...
    calcularDiferencias(): void {
        ...
        const fechaVigenciaDesdeActual = this.versionActual.fechaVigenciaDesde
            ...
            : 'Sin definir';
        const fechaVigenciaDesdeAnterior = this.versionAnterior
            .fechaVigenciaDesde
        ...
        this.diferenciasAtributos.push({
            ...
            cambiado:
                this.versionActual.fechaVigenciaDesde !==
                this.versionAnterior.fechaVigenciaDesde,
        });
        ...
        const fechaVigenciaHastaActual = this.versionActual.fechaVigenciaHasta
            ...
            : 'Sin definir';
        const fechaVigenciaHastaAnterior = this.versionAnterior
            .fechaVigenciaHasta
        ...
        this.diferenciasAtributos.push({
            ...
            cambiado:
                this.versionActual.fechaVigenciaHasta !==
                this.versionAnterior.fechaVigenciaHasta,
        });

        this.diferenciasAtributos.push({
            nombre: 'Apertura electrónica',
            valorActual: this.versionActual.aperturaElectronica ? 'Sí' : 'No',
            valorAnterior: this.versionAnterior.aperturaElectronica
                ? 'Sí'
                : 'No',
            cambiado:
                this.versionActual.aperturaElectronica !==
                this.versionAnterior.aperturaElectronica,
        });

        const tiposCompraActual = this.obtenerResumenTiposCompra(
            this.versionActual,
        );
        const tiposCompraAnterior = this.obtenerResumenTiposCompra(
        ...
        const objetosCompraActual = this.obtenerResumenObjetosCompra(
            this.versionActual,
        );
        const objetosCompraAnterior = this.obtenerResumenObjetosCompra(
            this.versionAnterior,
        );
        this.diferenciasAtributos.push({
        ...
        const incisosAnterior = this.obtenerResumenIncisos(
            this.versionAnterior,
        );
        this.diferenciasAtributos.push({
            nombre: 'Organismos',
    ...
    calcularDiferenciasRedacciones(): void {
        ...
        const prioridadesActuales = this.versionActual.redacciones.map(
            (r) => r.prioridad,
        );
        const prioridadesAnteriores = this.versionAnterior.redacciones.map(
            (r) => r.prioridad,
        );
        const todasPrioridades = Array.from(
            new Set([...prioridadesActuales, ...prioridadesAnteriores]),
        ).sort((a, b) => a - b);

        for (const prioridad of todasPrioridades) {
            const redaccionActual = this.versionActual.redacciones.find(
                (r) => r.prioridad === prioridad,
            );
            const redaccionAnterior = this.versionAnterior.redacciones.find(
                (r) => r.prioridad === prioridad,
            );
            ...
            const cambiada =
                redaccionActual &&
                redaccionAnterior &&
                redaccionActual.redaccion !== redaccionAnterior.redaccion;
    ...
    hayDiferencias(): boolean {
        const hayCambiosAtributos = this.diferenciasAtributos.some(
            (d) => d.cambiado,
        );
        const hayCambiosRedacciones = this.diferenciasRedacciones.some(
            (d) => d.cambiada || d.nueva || d.eliminada,
        );
