...
describe('UnidadesCompraSicePopupComponent', () => {
    ...
    beforeEach(async () => {
        organismoServiceSpy = jasmine.createSpyObj('OrganismoService', [
    ...
    it('debería ordenar y paginar las unidades obtenidas', () => {
        organismoServiceSpy.obtenerUCUsuarioOrganismo.and.returnValue(
            of([
        ...
        expect(
            organismoServiceSpy.obtenerUCUsuarioOrganismo,
        ).toHaveBeenCalledWith(usuario.id);
        expect(component.total).toBe(3);
        expect(component.unidadesCompra.map((uc) => uc.idInciso)).toEqual([
            1, 1, 2,
        ]);
