    ...
    implements OnInit
{
        ...
        if (usuario.tienePermisoTodas) {
            return [
                {
                    ...
                    ariaLabel:
                        'Eliminar roles asignados a todas las UC usuario id ' +
                        usuario.id,
            ...
            acciones = [
                ...
                },
                {
                    ...
                    ariaLabel:
                        'Asignar todas las UC del usuario id ' + usuario.id,
                    clase: 'btn btn-secondary',
                    icono: 'fa fa-sitemap',
                    permisos: ['GC_GESTION_USU.ALTA'],
                    accion: this.abrirAgregarPermisoTodasUcPopupParaUsuario.bind(
                        this,
                        usuario,
                    ),
                },
                {
                    ...
                    ariaLabel:
                        'Asignar por tipo de compra usuario id ' + usuario.id,
                    clase: 'btn btn-secondary',
    ...
    abrirAgregarPermisoTodasUcPopupParaUsuario(
        usuario: UsuarioPermisoAgrupadoDTO,
    ) {
        const comp = this.abrirPopup(NuevoUsuarioPopupComponent, 'Guardar', {
            initialState: {
        ...
        setTimeout(() => {
            comp.form
                .get('nroDocumento')
                ?.setValue(usuario.id.replace('uy-ci-', ''));
            comp.usuario = {
        ...
        this.usuarioRolesService
            ...
            .subscribe(() => {
                this.actualizarServ.mensajeCorrecto(
                    'Los roles han sido agregados de forma exitosa.',
                );
                this.buscar();
            });
    }
    ...
    guardarRolNuevoUsuarioParaTodasUc(data: any): void {
        ...
        setTimeout(() => {
            this.actualizarServ.confirmar(
                '¿Está seguro que desea agregar el rol a nivel de todas las UC que tiene el usuario en SICE?',
                () => {
                    this.usuarioRolesService
                        .agregarRolTodasUc(data.idUsuario)
                        .subscribe(() => {
                            this.actualizarServ.mensajeCorrecto(
    ...
    eliminarRolUsuarioEspecifico(permiso: any): void {
        ...
        this.actualizarServ.confirmar(
            '¿Está seguro que desea quitar los roles seleccionados?',
            () =>
                this.usuarioRolesService
                    .eliminarRol(permisoId)
                    .subscribe(() => {
                        this.actualizarServ.mensajeCorrecto(
                            'Se ha quitado el rol de forma exitosa.',
                        );
                        this.buscar();
                    }),
        );
    ...
    eliminarRolTodos(usuarioAgrupado: any): void {
        ...
        this.actualizarServ.confirmar(
            '¿Está seguro que desea quitar el rol?',
            () => {
                this.usuarioRolesService
                    .eliminarRol(permisoId)
                    .subscribe(() => {
                        this.actualizarServ.mensajeCorrecto(
                            'Se ha quitado la asignación del rol de forma exitosa.',
                        );
                        this.buscar();
                    });
            },
