...
export class AgregarModificarSeccionComponent
    ...
    implements OnInit, CanComponentDeactivate
{
    ...
    private verificarCapituloSeleccionado(): void {
        setTimeout(() => {
            ...
            if (state && state['capituloSeleccionado']) {
                this.agregarCapituloDesdeSeleccion(
                    state['capituloSeleccionado'],
                );
            } else {
                const historyState = window.history.state;
                if (historyState?.capituloSeleccionado) {
                    this.agregarCapituloDesdeSeleccion(
                        historyState.capituloSeleccionado,
                    );
    ...
    private verificarClausulaSeleccionada(): void {
        setTimeout(() => {
            ...
            if (state && state['clausulaSeleccionada']) {
                this.agregarClausulaDesdeSeleccion(
                    state['clausulaSeleccionada'],
                );
            } else {
                const historyState = window.history.state;
                if (historyState?.clausulaSeleccionada) {
                    this.agregarClausulaDesdeSeleccion(
                        historyState.clausulaSeleccionada,
                    );
                    const newState = { ...historyState };
    ...
    private agregarCapituloDesdeSeleccion(capitulo: SeccionCapituloDTO): void {
        const yaExiste = this.capitulosAgregados.some(
            (c) => c.capitulo?.id === capitulo.capitulo?.id,
        );

        if (yaExiste) {
            this.actualizarService.mensajeError(
                'El capítulo ya está agregado a la sección',
            );
            return;
        }
        ...
        this.actualizarService.mensajeCorrecto(
            'Capítulo agregado exitosamente',
        );
    }

    private agregarClausulaDesdeSeleccion(clausula: SeccionClausulaDTO): void {
        const yaExiste = this.clausulasAgregadas.some(
            (c) => c.clausula?.id === clausula.clausula?.id,
        );

        if (yaExiste) {
            this.actualizarService.mensajeError(
                'La cláusula ya está agregada a la sección',
            );
            return;
        }
        ...
        this.marcarFormularioTocado();
        this.actualizarService.mensajeCorrecto(
            'Cláusula agregada exitosamente',
        );
    }
            ...
            next: (seccion: SeccionDTO | undefined) => {
                ...
                this.form.patchValue({
                    ...
                    fechaVigenciaDesde: convertirFecha(
                        seccion.fechaVigenciaDesde,
                    ),
                    fechaVigenciaHasta: convertirFecha(
                        seccion.fechaVigenciaHasta,
                    ),
                });
    ...
    eliminarCapitulo(capitulo: SeccionCapituloDTO): void {
        const index = this.capitulosAgregados.findIndex(
            (c) => c.capitulo?.id === capitulo.capitulo?.id,
        );
        if (index > -1) {
    ...
    eliminarClausula(clausula: SeccionClausulaDTO): void {
        const index = this.clausulasAgregadas.findIndex(
            (c) => c.clausula?.id === clausula.clausula?.id,
        );
        if (index > -1) {
    ...
    moverCapituloArriba(capitulo: SeccionCapituloDTO): void {
        ...
        if (index > 0) {
            [
                this.capitulosAgregados[index - 1],
                this.capitulosAgregados[index],
            ] = [
                this.capitulosAgregados[index],
                this.capitulosAgregados[index - 1],
            ];
    ...
    moverClausulaArriba(clausula: SeccionClausulaDTO): void {
        ...
        if (index > 0) {
            [
                this.clausulasAgregadas[index - 1],
                this.clausulasAgregadas[index],
            ] = [
                this.clausulasAgregadas[index],
                this.clausulasAgregadas[index - 1],
            ];
    ...
    guardar(): void {
        ...
        const operacion = this.modoIngreso
            ...
            : this.seccionService.actualizarSeccion(
                  this.idSeccion,
        ...
        operacion.subscribe({
            ...
            error: (err) => {
                if (err?.error?.mensaje?.includes('denominación')) {
                    this.actualizarService.mensajeError(
                        'Ya existe una sección con esta denominación',
                    );
                } else {
                    this.actualizarService.mensajeError(
