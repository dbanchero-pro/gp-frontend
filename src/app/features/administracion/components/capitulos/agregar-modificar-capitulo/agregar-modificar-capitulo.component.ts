...
export class AgregarModificarCapituloComponent
    ...
    implements OnInit, CanComponentDeactivate
{
    ...
    private agregarClausulaDesdeSeleccion(clausula: CapituloClausulaDTO): void {
        ...
        if (yaExiste) {
            this.actualizarService.mensajeError(
                'La cláusula ya está agregada al capítulo',
            );
            return;
        }
            ...
            next: (capitulo: CapituloDTO | undefined) => {
                ...
                this.form.patchValue({
                    ...
                    fechaVigenciaDesde: convertirFecha(
                        capitulo.fechaVigenciaDesde,
                    ),
                    fechaVigenciaHasta: convertirFecha(
                        capitulo.fechaVigenciaHasta,
                    ),
                });
    ...
    guardar(): void {
        ...
        const operacion = this.modoIngreso
            ...
            : this.capituloService.actualizarCapitulo(
                  this.idCapitulo,
                  capitulo as CapituloDTO,
              );
        ...
        operacion.subscribe({
            next: () => {
                this.actualizarService.mensajeCorrecto(
                    this.modoIngreso
                        ...
                        : 'Capítulo actualizado exitosamente',
                );
                this.form.markAsPristine();
                this.volver();
            },
            error: (err) => {
                if (err?.error?.mensaje?.includes('denominación')) {
                    this.actualizarService.mensajeError(
                        'Ya existe un capítulo con esta denominación',
                    );
                } else {
                    this.actualizarService.mensajeError(
        ...
        this.actualizarService.confirmar(
            ...
            () => {
                this.capituloService
                    .aprobarCapitulo(this.idCapitulo)
                    .subscribe({
                        next: () => {
                            this.actualizarService.mensajeCorrecto(
                                'Capítulo aprobado exitosamente',
                            );
                            this.form.markAsPristine();
                            this.volver();
                        },
