...
export class ConsultaCapitulosComponent implements OnInit, AfterViewInit {
    ...
    obtenerAccionesCapitulo(capitulo: CapituloDTO): AccionBoton[] {
        ...
        acciones.push({
            ...
            icono: 'fa fa-list',
            ariaLabel:
                'Ver modelos que usan el capítulo ' + capitulo.denominacion,
            ...
            this.capituloService.eliminarCapitulo(capituloId).subscribe({
                ...
                error: () => {
                    this.actualizarService.mensajeError(
                        'Ocurrió un error al eliminar el capítulo.',
                    );
                },
            });
        });
    ...
    verHistorial(capitulo: CapituloDTO): void {
        ...
        this.router.navigate([
            '/administracion/capitulos/historial',
            capitulo.id,
        ]);
    }
    ...
    obtenerEstadoVigencia(capitulo: CapituloDTO): string {
        if (capitulo.estado === 'BORRADOR') {
            const hoy = new Date();
            const desde = capitulo.fechaVigenciaDesde
                ? new Date(capitulo.fechaVigenciaDesde)
                : null;
            const hasta = capitulo.fechaVigenciaHasta
                ? new Date(capitulo.fechaVigenciaHasta)
                : null;
