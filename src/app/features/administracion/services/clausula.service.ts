...
const crearObjetoCompraMock = (
    ...
    subfamilia: subfamiliaId
        ...
        : undefined,
    clase: claseId
        ? new ClaseDTO(claseId, claseId, claseDesc, familiaId, subfamiliaId)
        : undefined,
    subclase: subclaseId
        ? new SubclaseDTO(
              subclaseId,
              ...
              subclaseDesc,
              familiaId.toString(),
              subfamiliaId?.toString(),
              claseId?.toString(),
          )
        : undefined,
    articulo: articuloId
        ? new ArticuloServObraDTO(articuloId, articuloDesc)
        : undefined,
});
...
export class ClausulaService {
            ...
            redacciones: [
                {
                    ...
                    prioridad: 1,
                    redaccion:
                        '<p>Redacción inicial de referencia para validar render de listas.</p>',
        ...
        },
        {
            ...
            redacciones: [
                ...
                },
                {
                    ...
                    prioridad: 2,
                    redaccion:
                        '<p>La entrega deberá realizarse en el lugar indicado por la contratante, corriendo por cuenta del proveedor todos los gastos de:</p><ul><li><strong>Transporte</strong></li><li><strong>Seguro</strong></li><li><strong>Descarga</strong></li></ul>',
                    fechaCreacion: '2024-06-01',
                    usuarioCreacion: 'admin',
                    fechaModificacion: null,
                    usuarioModificacion: null,
    ...
    buscarClausulas(filtro: FiltroClausula): Observable<ClausulaDTO[]> {
        ...
        if (filtro.incisoId) {
            resultados = resultados.filter(
                (c) => c.organismo?.inciso?.id === filtro.incisoId,
            );
        }
        ...
        if (filtro.unidadEjecutoraId) {
            resultados = resultados.filter(
                (c) =>
                    c.organismo?.unidadEjecutora?.id ===
        ...
        if (filtro.familiaId) {
            resultados = resultados.filter((c) =>
                c.objetosCompra.some(
                    (oc) => oc.familia?.id === filtro.familiaId,
                ),
            );
        }
        ...
        if (filtro.subfamiliaId) {
            resultados = resultados.filter((c) =>
                c.objetosCompra.some(
                    (oc) => oc.subfamilia?.id === filtro.subfamiliaId,
                ),
            );
        }
        ...
        if (filtro.subclaseId) {
            resultados = resultados.filter((c) =>
                c.objetosCompra.some(
                    (oc) => oc.subclase?.id === filtro.subclaseId,
                ),
            );
        }
        ...
        if (filtro.articuloId) {
            resultados = resultados.filter((c) =>
                c.objetosCompra.some(
                    (oc) => oc.articulo?.id === filtro.articuloId,
                ),
            );
        }
    ...
    crearClausula(clausula: ClausulaDTO): Observable<ClausulaDTO> {
        const nuevoId =
            Math.max(...this.clausulasMock.map((c) => c.id || 0)) + 1;
        const nuevaClausula = {
            ...clausula,
            id: nuevoId,
    ...
    eliminarClausula(id: number): Observable<EliminarElementoResponseDTO> {
        ...
        const tieneVersionEditable =
            clausula.estado === EstadoElemento.BORRADOR;
                ...
                redacciones: [
                    {
                        ...
                        prioridad: 1,
                        redaccion:
                            '<p>Versión aprobada sin cambios sustantivos.</p>',
                ...
                redacciones: [
                    {
                        ...
                        prioridad: 1,
                        redaccion:
                            '<p>El proveedor deberá presentar una <strong>garantía de cumplimiento</strong> por el <em>8% del monto total del contrato</em>.</p>',
                ...
                redacciones: [
                    {
                        ...
                        prioridad: 1,
                        redaccion:
                            '<p>El proveedor deberá presentar una <strong>garantía de cumplimiento</strong> por el <em>5% del monto total del contrato</em>.</p>',
    ...
    obtenerVersionAnterior(clausulaId: number): Observable<ClausulaDTO | null> {
        return this.obtenerHistorialVersiones(clausulaId).pipe(
            ...
            map((versiones) => {
                ...
                const versionesOrdenadas = versiones.sort(
