...
describe('ElaborarPliegoService', () => {
    ...
    it('debe devolver historial mockeado con fechas validas', () => {
        ...
        expect(
            historial.every((tarea) => tarea.fecha instanceof Date),
        ).toBeTrue();
    });
    ...
    it('debe crear un pliego demo con los datos recibidos', () => {
        ...
        expect(pliego.unidadEjecutora.inciso?.descInciso).toBe(
            'Poder Ejecutivo',
        );
        expect(pliego.unidadEjecutora.descUnidadEjecutora).toBe(
            'Ministerio de Economia',
        );
