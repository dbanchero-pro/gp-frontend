...
export class AsignarUsuariosComponent
    ...
    implements OnInit, CanComponentDeactivate
{
    ...
    @ViewChild('agregarUsuarioTemplate', { static: false })
    agregarUsuarioTemplate: any;
    @ViewChild('modificarUsuarioTemplate', { static: false })
    modificarUsuarioTemplate: any;
    ...
    cargarUsuariosAsignados(): void {
        ...
        this.bandejaEntradaService
            .obtenerUsuariosAsignadosPorPliego(this.proceso.id)
            .subscribe({
    ...
    guardarNuevoUsuario(usuario: UsuarioAsignadoDTO): void {
        ...
        const yaAsignado = this.usuariosAsignados.some(
            (u: UsuarioAsignadoDTO) => u.id === usuario.id,
        );
        if (yaAsignado) {
            this.actualizarServ.mensajeInformacion(
                'El usuario ya está asignado al proceso',
            );
            return;
        }
    ...
    guardarUsuarioModificado(usuarioModificado: UsuarioAsignadoDTO): void {
        ...
        const index = this.usuariosAsignados.findIndex(
            (u: UsuarioAsignadoDTO) => u.id === usuarioModificado.id,
        );
        if (index !== -1) {
            this.usuariosAsignados[index] = usuarioModificado;
            this.actualizarServ.mensajeCorrecto(
                'Roles del usuario modificados correctamente',
            );
        }
    ...
    finalizar(): void {
        if (!this.proceso) {
            this.actualizarServ.mensajeError(
                'Error: no se ha cargado el proceso',
            );
            return;
        }
        ...
        const tieneEditorPrincipal = usuariosAsignados.some(
            (u: UsuarioAsignadoDTO) => u.roles.includes('Editor Principal'),
        );

        if (!tieneEditorPrincipal) {
            this.actualizarServ.mensajeInformacion(
                'Debe asignar un editor principal',
            );
            return;
        }
        ...
        this.bandejaEntradaService
            .asignarUsuariosYFinalizar(this.proceso.id, usuariosAsignados)
            .subscribe({
                next: () => {
    ...
    obtenerTextoTipoCompra(): string {
        ...
        const subtipoCompra =
            this.proceso.subtipoCompra?.descSubtipoCompra ?? '';
