...
export class IniciarPliegoComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        this.pliegoId = this.route.snapshot.params['id']
    ...
    private configurarCambioTipoBusqueda(): void {
        this.formularioFiltro
            .get('tipoBusqueda')
            ?.valueChanges.subscribe((tipoBusqueda) => {
                const incisoControl = this.formularioFiltro.get('incisoId');
                const unidadEjecutoraControl =
                    this.formularioFiltro.get('unidadEjecutoraId');
                ...
                if (tipoBusqueda === 'PO') {
                    ...
                    unidadEjecutoraControl?.setValue(null, {
                        emitEvent: false,
                    });
    ...
    private configurarCambioTipoCompra(): void {
        this.formularioFiltro
            ...
            ?.valueChanges.subscribe((tipoCompraId) => {
                ...
                if (tipoCompraId) {
                    const tipoSeleccionado = this.tiposCompra.find(
    ...
    buscar(): void {
        ...
        this.snapshotGenericService.save(IniciarPliegoComponent.SNAPSHOT_KEY, {
            filtro: valores,
    ...
    nuevaConsulta(): void {
        ...
        this.snapshotGenericService.clear(IniciarPliegoComponent.SNAPSHOT_KEY);
    }
    ...
    seleccionarModelo(modelo: ModeloDTO): void {
        if (!this.pliegoId) {
            this.actualizarService.mensajeError(
                'No se pudo identificar el pliego',
            );
            return;
        }
        ...
        this.actualizarService.confirmar(mensaje, () => {
            ...
            this.actualizarService.mensajeCorrecto(
                `El pliego ha sido iniciado con el modelo "${modelo.denominacion}"`,
            );
            this.volver();
        });
    }
    ...
    seleccionarPliego(pliego: PliegoDTO): void {
        ...
        this.actualizarService.confirmar(mensaje, () => {
            ...
            this.actualizarService.mensajeCorrecto(
                `El pliego ha sido iniciado con el pliego de la compra N° ${pliego.numeroCompra}/${pliego.anioCompra}`,
            );
            this.volver();
        });
    }
    ...
    verPliego(pliego: PliegoDTO): void {
        ...
        this.actualizarService.mensajeInformacion(
            'Funcionalidad de ver pliego en desarrollo',
        );
    ...
    esVigentePliego(pliego: PliegoDTO): boolean {
        const hoy = new Date();
        const desde = pliego.modelo?.fechaVigenciaDesde
            ? new Date(pliego.modelo.fechaVigenciaDesde)
            : null;
        const hasta = pliego.modelo?.fechaVigenciaHasta
            ? new Date(pliego.modelo.fechaVigenciaHasta)
            : null;
